	...
	for( my $i = 0; $i < $index; $i++){
		...
		if( $roll_lst[$i] =~ /CC$group/ ){
			...
			my $my_cc_ip = $ip_lst[$i];
			    ...
			    print "\n$my_cc_ip :: $ENV{'EUCALYPTUS'}/etc/init.d/eucalyptus-cc restart\n";
			    #STARTING CC
			    print("ssh -o StrictHostKeyChecking=no root\@$my_cc_ip \"$ENV{'EUCALYPTUS'}/etc/init.d/eucalyptus-cc restart\"\n");
			    $outstr = `ssh -o StrictHostKeyChecking=no root\@$my_cc_ip \"$ENV{'EUCALYPTUS'}/etc/init.d/eucalyptus-cc restart\"`;
			    
			    print $outstr;
			    if( $outstr =~ /done/ || $outstr =~ /Enabling IP/ ){
	        	        print "\nRestarted CC $my_cc_ip successfully !\n";
			    }else{
				if(!is_running("$ENV{'EUCALYPTUS'}/etc/init.d/eucalyptus-cc", $my_cc_ip)) {
				    print "\n[TEST_REPORT]\tFAILED to Restart CC $my_cc_ip !!\n";
				    $is_error = 1;
				}
				#exit(1);
			    };
...
sub restart_nc{
	...
	foreach my $my_nc_ip (@my_nc_ips){
		...
		print "\n$my_nc_ip :: $ENV{'EUCALYPTUS'}/etc/init.d/eucalyptus-nc restart\n";
		
		#Restart NCs
		print("ssh -o StrictHostKeyChecking=no root\@$my_nc_ip \"$ENV{'EUCALYPTUS'}/etc/init.d/eucalyptus-nc restart\"\n");
		$outstr = `ssh -o StrictHostKeyChecking=no root\@$my_nc_ip \"$ENV{'EUCALYPTUS'}/etc/init.d/eucalyptus-nc restart\"`;

		print $outstr;
                if( $outstr =~ /done/ || $outstr =~ /Enabling IP/ ){
			print "\nRestarted NC $my_nc_ip successfully !\n";
		}else{
		    if(!is_running("$ENV{'EUCALYPTUS'}/etc/init.d/eucalyptus-nc", $my_nc_ip)) {
			print "\n[TEST_REPORT]\tFAILED to Restart NC $my_nc_ip !!\n";
			$is_error = 1;
		    }
...
sub restart_cloud_components{
	...
	for( my $j = 0; $j < $index; $j++ ){
		...
		if( does_It_Have( $this_roll, "CLC") || does_It_Have( $this_roll, "SC") || does_It_Have( $this_roll, "WS") || does_It_Have( $this_roll, "VB")  ){
			...
			print "\n\n----------------------- Restart Cloud $this_ip [ $this_roll ] -----------------------\n";

			print "\n$this_ip :: $ENV{'EUCALYPTUS'}/etc/init.d/eucalyptus-cloud restart\n";
	
			#Restarting CLOUD
			print("ssh -o StrictHostKeyChecking=no root\@$this_ip \"$ENV{'EUCALYPTUS'}/etc/init.d/eucalyptus-cloud restart\"\n");
			$outstr = `ssh -o StrictHostKeyChecking=no root\@$this_ip \"$ENV{'EUCALYPTUS'}/etc/init.d/eucalyptus-cloud restart\"`;

			print $outstr;
			if( $outstr =~ /done/ ){
				print "Restarted CLOUD Components $this_ip successfully !\n";
			}else{
				sleep(3);
				...
				if( $check_ps =~ /eucalyptus-cloud/ ){
					print "\nRestarted CLOUD Components $this_ip successfully !\n";
				}else{
				    if(!is_running("$ENV{'EUCALYPTUS'}/etc/init.d/eucalyptus-cloud", $this_ip)) {
        	                        print "\n[TEST_REPORT]\tFAILED to Restart CLOUD Components $this_ip !!\n";
		        	        $is_error = 1;
				    }
				};
			};
