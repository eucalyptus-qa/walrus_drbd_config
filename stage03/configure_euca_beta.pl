...
for( my $i = 0; $i < @ip_lst; $i++ ){
	...
	if( !($this_distro eq "VMWARE" || $this_distro eq "WINDOWS") ){
		print "$this_ip : Running ./mod_euca_conf_beta.pl $this_distro $this_version $this_arch $this_source $stripped_roll $extra_ops\n"; 
		system("ssh -o StrictHostKeyChecking=no root\@$this_ip \"cd /root/seeds; perl ./mod_euca_conf_beta.pl $this_distro $this_version $this_arch $this_source $stripped_roll $extra_ops\" ");
	};
};
